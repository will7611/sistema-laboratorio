...
@php
    // Definimos el ID del usuario actual de forma segura
    $currentUserId = auth()->id();
    
    // 1. Contador para el globo rojo (solo no leídas)
    $cantidadNuevas = \Illuminate\Support\Facades\DB::table('sys_notifications')
        ->where('notifiable_id', $currentUserId)
        ->whereNull('read_at')
        ->count();
    
    // 2. Historial de las últimas 20 (Leídas y No Leídas)
    $notificacionesParaLista = \Illuminate\Support\Facades\DB::table('sys_notifications')
        ->where('notifiable_id', $currentUserId)
...
<div class="dropdown topbar-head-dropdown ms-1 header-item" id="notificationDropdown">
    <button type="button" class="btn btn-icon btn-topbar btn-ghost-secondary rounded-circle"
        ...
        <span class="position-absolute topbar-badge fs-10 translate-middle badge rounded-pill bg-danger" 
              id="notif-badge" style="{{ $cantidadNuevas == 0 ? 'display: none;' : '' }}">
            {{ $cantidadNuevas }}
        </span>
    </button>
    ...
    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0" aria-labelledby="page-header-notifications-dropdown">
        <div class="dropdown-head bg-primary bg-pattern rounded-top p-3">
            <div class="row align-items-center">
                ...
                <div class="col-auto">
                    <span class="badge badge-soft-light fs-13" id="notif-text-count"> {{ $cantidadNuevas }} Nuevas</span>
                </div>
            </div>
        </div>

        <div data-simplebar style="max-height: 300px;" class="p-2" id="notif-list-all">
            @if($notificacionesParaLista->count() > 0)
                @foreach($notificacionesParaLista as $notifDB)
                    @php
                        $data = json_decode($notifDB->data, true);
                        $claseEstado = is_null($notifDB->read_at) ? 'bg-light' : ''; 
                    @endphp

                    <div class="text-reset notification-item d-block dropdown-item border-bottom {{ $claseEstado }}">
                        <div class="d-flex">
                            ...
                            <div class="flex-grow-1">
                                <a href="{{ url('/notificaciones/orden/' . ($data['proforma_id'] ?? '')) }}" class="stretched-link">
                                    <h6 class="mt-0 mb-1 fs-13 fw-semibold">Orden #{{ $data['proforma_id'] ?? 'N/A' }}</h6>
                                </a>
                                <div class="fs-12 text-muted">
                                    <p class="mb-1">Paciente: {{ $data['paciente_nombre'] ?? 'N/A' }}</p>
                                </div>
                                <p class="mb-0 fs-11 text-muted">
                                    <i class="mdi mdi-clock-outline"></i> {{ \Carbon\Carbon::parse($notifDB->created_at)->diffForHumans() }}
                                </p>
                            </div>
                        </div>
                    </div>
                @endforeach
            @else
                <div class="text-center p-4" id="notif-empty-state">
                    <h6 class="fs-14 fw-medium text-muted">No se encontraron notificaciones.</h6>
                </div>
            @endif
        </div>
    </div>
